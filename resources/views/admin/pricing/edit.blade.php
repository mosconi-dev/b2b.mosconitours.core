<x-app-layout>
    <x-slot name="header">
        <x-page-heading title="Edit rule" subtitle="{{ $rule->serviceLine() }} — {{ $rule->amountLabel() }}">
            <x-slot name="icon">
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 px-6 py-4">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Saving changes what every future search is charged. Bookings already priced keep the
                    rule as it was — each one stored its own copy — so this does not reach backwards.
            ...
            <form method="POST" action="{{ route('admin.pricing.rules.update', $rule) }}" class="px-6 py-5">
                @csrf @method('PUT')

                @include('admin.pricing._field-guide', ['audience' => 'office'])

                @include('admin.pricing._rule-fields', ['rule' => $rule, 'audience' => 'office', 'idPrefix' => ''])
                ...
                <div class="mt-4 flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.pricing.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700">Cancel</a>
                </div>
            </form>
        </div>
    </div>
